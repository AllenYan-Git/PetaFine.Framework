...
namespace Infrastructure
{
    ...
    public class EnumHelper
    {
        ...
        /// <param name="e"></param>
        /// <returns></returns>
        public static string GetEnumDescription(Enum e, Enum language = null)
        {
            if (e == null)
            {
                return "";
            ...
            foreach (string enumValue in valueArray)
            {
                ...
                {
                    ret += attrs[0].Description + ENUM_TITLE_SEPARATOR;
                }
            }
            return ret.TrimEnd(ENUM_TITLE_SEPARATOR.ToArray());
        }
        ...
        /// <summary>
        /// 获取枚举列表的Description属性说明
        /// add by   nxj
        /// 20150103
        /// </summary>
        /// <typeparam name="T">枚举</typeparam>
        /// <returns>键值和Description说明</returns>
        public static Dictionary<int, string> GetEnumList<T>()
        {
            ...
            foreach (var item in typeof(T).GetFields())
            {
                ...
                if (arr.Length > 0)
                {
                    ...
                    Dic.Add((int)Enum.Parse(typeof(T), item.Name), ((DescriptionAttribute)arr[0]).Description);
                }
            }
            return Dic;
        } 
        ...
        /// <summary>
        /// 获取枚举说明列表返回List《SelectListItem》
        /// </summary>
        /// <typeparam name="T">枚举</typeparam>
        /// <param name="isAll">是否在第一项加上“全部”</param>
        /// <param name="text">当isAll==true时，第一项显示的文字</param>
        /// <param name="value">当isAll==true时，第一项的值</param>
        /// <returns></returns>
        public static List<SelectListItem> GetSelectItemByEnum<T>(bool isAll = false, string text = "全部", string value = "0")
        {
            var list = new List<SelectListItem>();
            if (isAll) { list.Add(new SelectListItem() { Text = text, Value = value, Selected = true }); }
            var dict = GetEnumList<T>();
            int i = 0;
            foreach (var item in dict)
            {
                list.Add(new SelectListItem()
                {
                    ...
                    Selected = (isAll ? false : (i == 0 ? true : false))
                }); 
                i++;
            } 
            return list;
        ...
        public static int GetEmunValueByDescription<T>(string descript)
        {
            int value =-99;
           var properties= typeof(T).GetFields();
           foreach (var item in properties)
           {
               var attributes = item.GetCustomAttributes(typeof(DescriptionAttribute),false) as DescriptionAttribute[];;
               if (attributes.Length > 0)
               {
                   foreach (var attr in attributes)
                   {
                       if (attr.Description == descript)
                       {
                           return (int)Enum.Parse(typeof(T), item.Name);
                       }
                   }
               }
           }
